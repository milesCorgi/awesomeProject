  ...
  <div>
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <span>
            {{ scope.row.PublishedAt | dateFormat}}
          </span>
        </template>
      </el-table-column>
...
export default {
  ...
  created () {
    ...
    this.page.pageSize = storage.get('two-set-info-youtube-list-pageSize')
    if (this.page.pageSize === undefined || this.page.pageSize === '') {
      this.page.pageSize = 10
    }

    this.page.currentPage = storage.get('two-set-info-youtube-list-currentPage')
    if (this.page.currentPage === undefined || this.page.currentPage === '') {
      this.page.currentPage = 1
    }

    this.getInfo()
    this.currentRow = storage.get('two-set-info-youtube-list-currentRow')
  ...
  methods: {
    ...
    handleHighlightCurrentChange (row) {
      ...
      storage.set('two-set-info-youtube-list-currentRow', this.currentRow)
    ...
    handleSizeChange (val) {
      ...
      storage.set('two-set-info-youtube-list-pageSize', val)
    ...
    handleCurrentPageChange (val) {
      ...
      storage.set('two-set-info-youtube-list-currentPage', val)
      console.log(`当前页: ${val}`)
      this.updateCurrentPageInfo()
    },
    isMobile () {
    ...
    getInfo () {
      ...
      this.$http.post('/api/query_youtube_video_list',
        {
